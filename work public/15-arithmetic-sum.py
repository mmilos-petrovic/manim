...
class ArithmeticSum(Scene):
    def construct(self):
        ...
        # TITLE
        title = Text("Sum of arithmetic progression", font_size = 26, color = BLUE).to_edge(UP)
        self.add(title)
        ...
        G1 = VGroup(a1, a2, a3, a4, d21, d31, d32, d41, d42, d43).next_to(sum, 2 * DOWN).shift(2 * LEFT)
        G1A = VGroup(a1, a2, a3, a4)
        G1D = VGroup(d21, d31, d32, d41, d42, d43)
        S4str = MathTex("S_4 =", font_size = 40).next_to(G1, 6 * LEFT)
        ...
        for e in G2D:
             e[1].rotate(PI)
        G2DM = G2D.copy().shift(a1.get_critical_point(UR) - G2D.get_critical_point(UL)).shift(0.17 * RIGHT)
        ...
        # 2 * S4 = 2 * 4 * a + 4(4-1)d
        tex = MathTex([two, S4], "=", [ [two1, dot, four, dot, a], "+", [four1, "(", four2, "-", "1", ")", d ]], font_size = 42).align_to(S4str2, LEFT).shift(2.5 * DOWN)
        ...
        self.wait(0.5)
        self.play(tex.animate().shift([-tex.get_center()[0], 0, 0]), run_time = 0.5)
