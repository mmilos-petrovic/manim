...
class Hexagon(Scene):
    def construct(self):
        self.wait(1)
        title = Text("Area of a hexagon formula", font_size = 36, color = BLUE).to_corner(UL)
        self =  Tools.add_with_click_silent(self, title, 0)
        
        trinagle_shift = [4.5,1,0]
        hexagon_formula_shift = [3, 0, 0]

        hexagon = RegularPolygon(n = 6, color = BLUE, fill_opacity = 0.5, stroke_width = 6).scale(2).shift(4 * LEFT + 0.5 * DOWN)
        points = {}
        labels = {}
        labels2 = {}
        ...
        self.wait(1)
        tmp1 = Triangle(color = YELLOW, fill_opacity = 0.5).scale(0.3)
        tmp2 = MathTex(r"=\dfrac{a^2 \sqrt{3}}{4}", color = YELLOW, font_size = 32).next_to(tmp1, RIGHT*1.7)
        self.add(VGroup(tmp1, tmp2).next_to(triangle, UP * 1.5))
        
        # solution
        sol1 = RegularPolygon(color = BLUE, fill_opacity = 0.5).scale(0.3)
        ...
        sol21 = RegularPolygon(color = BLUE, fill_opacity = 0.5).scale(0.3).next_to(sol1, 4 * DOWN)
        ...
        sol31 = RegularPolygon(color = BLUE, fill_opacity = 0.5).scale(0.3).next_to(sol21, 4 * DOWN)
