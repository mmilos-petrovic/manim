...
class LCMShort(Scene):
    def construct(self):
        title = MarkupText(f'Find <span fgcolor="{YELLOW}">LCM(70, 84, 150)</span> using',  
                           font_size=24, 
                           color=BLUE).to_edge(UP).shift(0.9 * DOWN)
        title2 = MarkupText(f'prime factorisation.',  
                           font_size=22, 
                           color=WHITE).next_to(title, DOWN)
        ...
        # step 1
        self = Tools.add_step_with_sound(self, 1, "write each number as a product\nof prime factors.", UL, 3.5 * DOWN, 1)
        ...
        ).arrange_in_grid(4, 13, buff= (0.4, 0.7)).shift(0.5 * DOWN).shift(RIGHT)
        self.wait(1)
        ...
        # step 2
        self = Tools.add_step_with_sound(self, 2, "match primes vertically.", UL, 3.5 * DOWN, 1)
        allNumbers2 = VGroup(
            ...
            empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(), empty.copy(),
        ).arrange_in_grid(4, 13, buff= (0.3, 0.7)).shift(0.5 * DOWN)
        ...
        # step 3
        self = Tools.add_step_with_sound(self, 3, "bring down numbers in each\ncolumn.", UL, 3.5 * DOWN, 1)
        box_time = 0.3
        num_time = 0.2
        ...
        # step 4
        self = Tools.add_step_with_sound(self, 4, "multiply last row.", UL, 3.8 * DOWN, 1)
        rez = Text("LCM = 2100", font_size=font_size, color = BLUE).next_to(allNumbers2, 4 * DOWN)
