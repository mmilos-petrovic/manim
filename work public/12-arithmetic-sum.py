...
class ArithmeticSum(Scene):
    def construct(self):
        
        title = Text("Sum of first n integers formula", font_size=28, color=BLUE).to_corner(UL).shift(0.5 * RIGHT)
        self.add(title)
        ...
        group1.arrange(RIGHT).to_edge(UL, buff = 1).shift(1.5 * RIGHT + 0.7 * DOWN)
        ...
        gSquares = VGroup()
        for i in range(max):
            sqPack = squarePack(i + 1, a).next_to(group1[2 * i], 4 * DOWN)
            gSquares.add(sqPack)
        ...
        self.play(gSquares2.animate.shift(RIGHT * 5), run_time = 1.5)
        for i in range(3):
        ...
        braceLeft = BraceBetweenPoints(
              ...
              buff = 0.5)
        braceLeftStr = braceLeft.get_text("6 + 1", buff=0).set_color(WHITE).rotate(PI/2)
        ...
        braceDownStr = braceDown.get_text("6").set_color(WHITE)
        ...
        sol1 = MathTex("6 \cdot ( 6 + 1 )").next_to(equal2, 2 * RIGHT)
        self.wait(1)
        self.play(Write(sol1))
        self.wait(1)
        
        # MOVE TO TOP
        sol2 = MathTex(r"\frac{ 6 \cdot (6 + 1)}{ 2 } ").next_to(equal2, RIGHT)
        self.play(
            AnimationGroup(
        ...
        # MOVE RIGHT
        groupMove = VGroup(group1[10], equal, sol2)
        self.play(groupMove.animate().shift(1.2 * RIGHT), run_time = 1)
        
        # ADD ...
        group1[10] = MathTex(r".\,.\,.\,+")
        self = Tools.create_with_sound(self,  group1[10].next_to(group1[9]), 1, 0.3, 0.4)
        
        self.wait(1)
        
        # REPLACE N
        sol3 = MathTex(r"\frac{ n \cdot (n + 1)}{ 2 } ").next_to(equal, RIGHT)
        sol3[0][0].set_color(BLUE)
        sol3[0][3].set_color(BLUE)
        self.play(FadeOut(groupMove[0]), 
                  Create(MathTex("n", color = BLUE).next_to(group1[10], RIGHT)),
