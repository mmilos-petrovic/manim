...
class TrapezoidArea(Scene):
    def construct(self):
        ...
        B = np.array((3.2, -2.3, 0.0))
        CC = np.array((1.5, 0.5, 0.0))
        D = np.array((-1.5, 0.5, 0.0))
        H = np.array((-1.5, -2.3, 0.0))
        H2 = D * 0.5 + H * 0.5
        M1 = A * 0.5 + D * 0.5
        M2 = B * 0.5 + CC * 0.5
        R = B + CC - D
        ...
        # INTRO
        title1 = Text("Area of a trapezoid", font_size = 36, color = BLUE).to_edge(UP)
        self.add(title1)
        ...
        #QUESTION
        solution= Tex(r'\text{Area = }', r'$\dfrac{h}{2}$', r'$~\cdot~$', r'(a+b)', font_size=42, color = YELLOW).to_edge(UP, buff = 1.5)
        ...
        part2 = Polygon(*[R + np.array((0, 0.03, 0)), R2, R + H2 - H])
        ...
        self.play(
            rectangleGroup.animate().shift(A/8 - R/8).shift(UP), run_time = 1
        )
